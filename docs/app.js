...
function buildInvoiceItems() {
  ...
  servicesInCart.forEach((item) => {
    output.push(
      `<div class="invoice-name-price">${item.name}<p></p><p>${item.price}</p></div>`
    );
  });
...
const serviceCardContainerElm = document.getElementById(
  ...
  filteredServices.forEach((service) => {
    ...
    output.push(`
            <div class="service-card" id="${service.id}">
                ...
                <button class="button button--invoice ${
                  isInCart ? "button--remove" : ""
                }" data-index="${service.index}">${
      isInCart ? removeBtnPhrase : addBtnPhrase
    }</button>
            </div>
        `);
  });
...
function buildQuiz() {
  ...
  stepTextElm.innerHTML = `Step ${currentQuestion + 1} of ${
    totalQuestions + 1
  }`;
